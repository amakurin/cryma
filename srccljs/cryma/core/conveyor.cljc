...
(defn unordered-pipeline-async
  ...
  ([n to af from close?]
   ...
   (let [jobs (chan n)
         async (fn [job]
                 (if (nil? job)
                   (do (when close? (async/close! to)) nil)
                   (let [res (chan 1)]
                     (af job res)
                     (go-loop []
                       (let [v (<! res)]
    ...
    (loop [specs specs in inner-in]
      (if-let [{:keys [parallelism buffer keep-messaging-order?]
        ...
        (let [pipeline (if keep-messaging-order? async/pipeline-async unordered-pipeline-async)
          ...
          (pipeline parallelism spec-out (make-pipeline-handler spec) in true)
          (recur (rest specs) spec-out))
        (async/pipe in inner-out true)))
    (async/pipe inner-out out false)
    (when msg-on-rebuilt?
      (put! in msg-on-rebuilt?))))

(defn pipe-conveyor-ctor [in out & [opts-map]]
  (let [state (atom [])
