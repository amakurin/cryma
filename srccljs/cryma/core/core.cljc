(ns cryma.core.core
  ...
  (:require
    ...
    [cryma.core.msg-filters :as f]
    [cryma.core.conveyor :as convey]
    #?(:cljs
...
#?(:clj
   (deftype DroppingBuffer [^LinkedList buf ^long n logger]
     ...
     (add!* [this itm]
       (if-not (>= (.size buf) n)
         (.addFirst buf itm)
         (logger :warn "Buffer overflows size " n ". Dropping item: " itm))
       this)
     (close-buf! [_])
     Counted
     (count [_] (.size buf)))
...
(defn create-event-chan [core]
  (let [in (chan (dropping-buffer 2048 (:logger core)))
        out (async/chan 1)
        pipe-conveyor (convey/pipe-conveyor-ctor in out)]
    {:in            in
...
(defn create-request-chan [_]
  (let [in (chan 2048)
        out (async/chan 1024)
        pipe-conveyor (convey/pipe-conveyor-ctor in out {:unordered-pipelines? true})]
    {:in            in
     :pipe-conveyor pipe-conveyor
     :out           out
     :servers       []}))
...
(defn -register-pipeline
  ...
  (let [conveyor (read-state core internals-store [internal-key :pipe-conveyor])]
...
(defn -subscribe-event
  ...
  (let [src-mult (read-state core internals-store [:event-chan :mul])
    ...
    (when handler (attach-handler core :event (assoc spec :channel channel)))))
...
(defn -register-server
  ...
  (let [channel (new-or-pipe channel)
    ...
    (when handler (attach-handler core :request (assoc spec :channel channel)))
    (transact-state
      core
...
(defrecord Core []
  ...
  (register-module
    ...
    (let [module-instructions
          [[api/event-pipelines-key
            (partial -register-pipeline :event-chan core)]
           [api/request-pipelines-key
            (partial -register-pipeline :request-chan core)]
           [api/event-subs-key
  ...
  (map->Core
    (merge {api/id-key           :core
            ...
            api/post-start-step-action-key
                                 #'api/register-module
            core-state-key       (atom {})
            core-instance-id-key (str (tools/guid))}
           m)))
