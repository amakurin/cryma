(ns cryma.core.in-memory-session-manager
  (:require
...
(defn -update-session
  ...
  (let [state (state-key manager)
        k (first korks)
        path (tools/korks-path k (rest korks))]
...
(defrecord InMemorySessionManager []
  ...
  (start [manager]
    (let [event-subs
      ...
      (->
        manager
        (tools/->if watcher-control-chan assoc watcher-control-chan-key watcher-control-chan)
        (tools/->if (seq event-subs) assoc api/event-subs-key event-subs))))
