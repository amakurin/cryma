(ns cryma.core.transport
  "Sente based websocket/ajax transport
  NOTES:
  * Sente sends over UIDs. This transport uses 3-component UIDs (user,session,client) produced by ring-request->uid function. Each component could be overriden by transport component config.
  * In most common case we have such an hierarchy (-> means has zero or many):
  USER (busines/logic user) -> Sessions (http sessions with devices) -> Clients (e.g. browser-tabs).
